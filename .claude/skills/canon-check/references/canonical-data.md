# Canonical Data — Northwatch Wardens

## Valid Northreach Locations

Any location name **not** on this list is a flag.

| Location | Associated Adventure / Purpose |
|----------|-------------------------------|
| Waystone Inn | Guild HQ, mission hub (all adventures) |
| Welton | Wolves of Welton |
| Westly's Farm | Wolves of Welton |
| Shepherd's Crook Inn | Welton area |
| Pinebrook | Peril in Pinebrook |
| Palebank Village | Frozen Sick |
| Croaker Cave | Frozen Sick |
| Salsvault | Buried Aeorian ruins — source of the Echo mystery |
| Temple of the Dragonknights | Capstone adventure (northwest mountains) |
| Noke's Tower | Wild Sheep Chase |

## Acceptable Off-Map References

These exist in the world of Aevoria but the party doesn't visit them in Season One. References are acceptable; making them a destination is not.

- Solaris
- Nullwood / Vaeltharyn
- Stonebound Depths / Khardûn-Tharum
- Vharoxis
- Solace Nexus
- Divinity's Beacon

**World:** Aevoria | **Region:** Northreach (frontier territory)

---

## Core Guild NPCs (always canonical)

These characters are always valid — no need to check against the roster.

| NPC | Role |
|-----|------|
| Marshal Brenna Thorne | Field commander |
| Steward Mara Fenwick | Quartermaster |
| Lorewarden Elric Vael | Arcane scholar |

**Authoritative NPC roster:** `Season 1/Campaign Assets/DM Guild Roster.md`

---

## Player-Facing Content Rules

Files in `build/players-guide-toc.json` must **never** contain:

| Banned pattern | Use instead |
|----------------|-------------|
| `[text](path/file.md)` | `**Chapter X: Title**` |
| `Season 1/Adventures/` | `Available from your DM` |
| `Premade PCs/` | `Available from your DM` |
| Any GitHub URL or repo reference | Remove entirely |

---

## Tone: Flag These Patterns

### Emotion dictation (in read-aloud text)
- "you feel scared / uneasy / dread / hopeful"
- "you sense something is wrong"
- "you are overcome with"
- "you notice a feeling of"

### Generic fantasy clichés
- "ancient evil"
- "mystical power" / "mystical energies"
- "dark forces awakening"
- "eldritch energies" / "eldritch power"
- "arcane energy" (when used vaguely)
- "chosen one" / "destiny"

### Overwrought prose
- More than 3 adjectives modifying a single noun
- Sentences with 4+ adverbs

## Tone: What's Correct

- Specific, tactile sensory details ("the mud is ankle-deep, cold, and smells of rot")
- Understated menace ("the silence where birdsong should be")
- Observations without interpretation ("three crows sit on the fence post. None of them have moved.")
- Frontier realism: things are hard, scarce, and earned
