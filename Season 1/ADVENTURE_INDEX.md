# Adventure Index: Quick Reference Table

**Purpose:** Use this table to select and prepare adventures efficiently. All information is condensed for quick lookup. For full adventure details, see Chapter 5 adventure modules.

**How to Use:**
1. **Select by Level Range** — Find your party's level
2. **Check Mystery Rating** — Decide impact level you want
3. **Review Type** — Choose adventure style preference
4. **Note Duration** — Plan session count accordingly
5. **Go to Module** — Link takes you to full adventure

---

## Complete Adventure Index

| Adventure | Level | Sessions | Mystery★ | Type | Key NPCs | Path | Echo Clue | Consequences |
|-----------|-------|----------|---------|------|----------|------|-----------|--------------|
| **[Opening Adventures]** | — | — | — | — | — | — | — | — |
| Open Skirmish | 1-2 | 1 | ★☆☆☆☆ | Combat | None | Combat intro | None | Party cohesion |
| The Morning After | Any | 0.5 | ★☆☆☆☆ | Social | Brenna, Elric, Mara | Social setup | None | Guild introduction |
| Wolves Contract | Any | 0.5 | ★☆☆☆☆ | Briefing | Brenna, Dornan | Plot setup | Potential threat | Adventure pickup |
| **[Core Adventures]** | — | — | — | — | — | — | — | — |
| Wolves of Welton | 1-3 | 2-3 | ★★☆☆☆ | Investigation | Dornan, Werethekau (Bolt/Flame) | Negotiation+Combat | Intelligence awakening | Council politics, pack choice |
| Frozen Sick | 2-4 | 3-4 | ★★★★★ | Horror+Exploration | Tulgi, Urgon, Elro | Deep dive | Aeorian origin/Salsvault | Plague response, source knowledge |
| Temple of the Dragonknights | 4-5 | 4-5 | ★★★★☆ | Combat+Infiltration | Cult Leader, Venomfang | Escalation | Faction exploitation | Dragon/Cult fate |
| Wild Sheep Chase | 1-2 | 1-2 | ★★☆☆☆ | Comedy | Finethir, Noke | Levity | Magic instability | Loot, memorable story |
| Peril in Pinebrook | 1-3 | 2-3 | ★★★☆☆ | Investigation | Garthok, traders | Community focus | Fragility + manifestation | Settlement fate |

---

## Adventure Selection Guide

### By Difficulty Level

**Levels 1-2 (Lowest):**
- Open Skirmish (combat inciting incident)
- Wild Sheep Chase (comic relief, magic theme)

**Levels 1-3 (Early):**
- Wolves of Welton (investigation focus, moral choice)
- Peril in Pinebrook (side trek, community focus)
- The Morning After (social setup)

**Levels 2-4 (Mid):**
- Frozen Sick (major horror/mystery push)

**Levels 4-5 (Advanced):**
- Temple of the Dragonknights (climactic, faction warfare)

### By Type of Adventure

**Investigation** (Figure it out):
- Wolves of Welton
- Frozen Sick
- Peril in Pinebrook

**Combat** (Fight it):
- Open Skirmish
- Temple of the Dragonknights
- (Wolves of Welton includes combat)

**Social/Negotiation**:
- Wolves of Welton (council voting)
- The Morning After (guild introduction)
- Peril in Pinebrook (town politics)

**Exploration**:
- Frozen Sick (ruins crawl)
- Wild Sheep Chase (wilderness chase)

**Comedy/Levity**:
- Wild Sheep Chase (pure comedy)
- Open Skirmish (light combat intro)

### By Mystery Progression

**Light Mystery** (★★☆☆☆ or less):
- Open Skirmish, The Morning After
- Wild Sheep Chase
- Wolves of Welton (first clue)

**Moderate Mystery** (★★★☆☆):
- Peril in Pinebrook
- Wolves of Welton (if second/third adventure)

**Major Mystery** (★★★★☆ or higher):
- Frozen Sick (★★★★★ — THE mystery revelation)
- Temple of the Dragonknights (★★★★☆ — faction exploitation)

**Recommended Play Order** (builds mystery naturally):
1. Open Skirmish + Wolves of Welton (intro, first clue)
2. Wild Sheep Chase (palate cleanser, magic instability hint)
3. Peril in Pinebrook (mid-game, secondary mystery)
4. Frozen Sick (revelation, answers main question)
5. Temple of the Dragonknights (capstone, faction conflict)

**Flexible Play Order** (adventures work in any sequence):
- All core adventures are order-independent
- Mystery builds naturally regardless of sequence
- Late-game understanding changes based on play order
- This is intentional design

---

## Key NPC Quick Reference

| Adventure | Primary NPC | Role | Archetype |
|-----------|-------------|------|-----------|
| Wolves of Welton | Dornan (shepherd) / Werethekau (alpha wolf) | Victim / Antagonist | Innocent / Sympathetic villain |
| Frozen Sick | Tulgi (healer) / Elro (scholar) | Helper / Guide | Ally / Mentor |
| Temple DK | Cult Leader / Venomfang (dragon) | Mastermind / Force | Manipulator / Amoral predator |
| Wild Sheep Chase | Finethir Shinebright / Noke (wizard) | Victim / Antagonist | Comedy victim / Arrogant mage |
| Peril Pinebrook | Garthok (elder) / Mysterious trader | Helper / Complication | Ally / Mystery |

*For full NPC details, stat blocks, and secrets: See Chapter 4 - DM Guild Roster*

---

## Echo Rating Explanation

**★☆☆☆☆ (Minimal):** Adventure hints at disturbance but doesn't explain source. Players get: "Something strange here."

**★★☆☆☆ (Secondary):** Adventure reveals one aspect of Echo (intelligence awakening, magic instability). Players wonder: "Is this connected?"

**★★★☆☆ (Tertiary):** Adventure confirms pattern. Players think: "These events are linked somehow."

**★★★★☆ (Major):** Adventure provides significant explanation. Players understand: "A larger force is at work."

**★★★★★ (Central):** Adventure reveals core mystery. Players know: "Aeorian magic from ancient ruins is awakening and destabilizing the frontier."

---

## Session Count Guidance

**By Adventure:**
- **Open Skirmish** — 1 session (30-60 min, combat encounter)
- **The Morning After** — 0.5 session (30-45 min, upfront ceremony)
- **Wild Sheep Chase** — 1-2 sessions (light, can be played quick or extended)
- **Wolves of Welton** — 2-3 sessions (investigation + combat)
- **Peril in Pinebrook** — 2-3 sessions (shorter, good mid-campaign)
- **Frozen Sick** — 3-4 sessions (longest, most complex)
- **Temple of the Dragonknights** — 4-5 sessions (epic capstone, multiple paths)

**Planning Tips:**
- Expect 1-2 hours per session
- Longer games run longer
- Investigation-heavy adventures expand naturally
- Combat rounds take time (budget accordingly)
- Add 30-60 min for debrief + next mission briefing

---

## Scaling for Party Size

All adventures support 2-5 player scaling. Typical adjustments:

**2 Players:** Reduce enemy count by 25%, increase NPC allies, provide additional healing.

**3-4 Players:** Standard adventure as written.

**5+ Players:** Add reinforcements (20-30% more enemies), increase HP on solo threats, expand battle areas.

*See individual adventure modules for specific scaling tables.*

---

## Consequence Tracking

**After Each Adventure, Note:**
1. **Major Outcomes** — What did players choose?
   - *Example:* "Voted to spare Bolt's pack; Flame fled north"
2. **Material Gains** — What did party acquire?
   - *Example:* "Warden rank advancement; +200 gp"
3. **Relationship Changes** — How did NPCs react?
   - *Example:* "Brenna appreciates thorough investigation; Wardens respect party"
4. **Mystery Progress** — What did players learn?
   - *Example:* "Discovered Aeorian spores are source; know Salsvault exists but location unknown"
5. **World Changes** — What happens next session?
   - *Example:* "Welton wolf pack now allies; Flame organizing eastern packs as rival faction"

**Consequence Sheet:** Use table in Chapter 4 - Campaign Tracker to track these decisions.

---

## Choosing Your Next Adventure

### Decision Matrix

**"Party is Level 2. What should I run?"**
→ Wolves of Welton (standard) or Wild Sheep Chase (if want lighthearted)

**"Party is Level 4. Haven't run Frozen Sick yet."**
→ Run Frozen Sick (mystery revelation time) OR Temple (if more combat-focused)

**"Party wants to skip Wolves of Welton."**
→ Go directly to any other adventure (all order-independent). Come back to Wolves later if you want.

**"Party is bored with current arc."**
→ Shift tone: If in serious investigations, run Wild Sheep Chase next. If they want challenge, go to Temple.

**"I need to finish campaign soon."**
→ Run Frozen Sick (mystery answered) then skip to Temple (capstone). Peril is optional.

**"I want maximum mystery drama."**
→ Run Frozen Sick LAST (maximum revelation). Start with Wolves, add others, save Frozen Sick for penultimate session.

---

## Preparation Checklist: Before Running Any Adventure

- [ ] Read adventure module fully
- [ ] Review Mystery Box (what's the Echo clue?)
- [ ] Read NPC descriptions and role-play notes
- [ ] Note 2-3 player approach possibilities
- [ ] Check scaling table for your party size
- [ ] Prepare maps (or note descriptions for theater-of-mind)
- [ ] Review Chapter 6 secrets for locations involved
- [ ] Prepare consequence notes (how will this adventure ripple forward?)
- [ ] Set up music/atmosphere (optional but recommended)

---

## Appendix: Adventure Location Map

```
Waystone Inn (Campaign Hub)
    │
    ├─ [2 days SW] → Welton (Wolves of Welton)
    │                  └─ Shepherd's Crook Inn
    │                  └─ Westly's Farm
    │
    ├─ [5 days NE] → Palebank Village (Frozen Sick)
    │                  └─ Jolly Dwarf Tavern
    │                  └─ Croaker Cave (bandit hideout)
    │                  └─ North to Salsvault (ancient ruins)
    │
    ├─ [3 days SE] → Pinebrook (Peril in Pinebrook)
    │                  └─ The Crossroads Inn
    │
    ├─ [1 day W]   → Noke's Tower (Wild Sheep Chase)
    │
    └─ [7 days NW] → Temple of the Dragonknights (capstone)
```

*For complete regional geography, see Chapter 7 - Appendix: Travel & Distance*

---

## Quick Links to Adventures

📍 **See full adventure modules in Chapter 5:**
- **Opening Adventures:** Chapter 5 (Starting Point)
- **Wolves of Welton:** Chapter 5 (Investigation adventure)
- **Frozen Sick:** Chapter 5 (Horror/Revelation adventure)
- **Temple of the Dragonknights:** Chapter 5 (Combat adventure)
- **Wild Sheep Chase:** Chapter 5 (Comedy adventure)
- **Peril in Pinebrook:** Chapter 5 (Community adventure)

---

## Notes Field: Track Your Campaign

Use space below to note which adventures you've run, party discoveries, and NPCs they've met:

```
Session:  1-2 | Adventure: Open Skirmish + Wolves of Welton
Notes: Party bloodied; negotiated with Bolt; Flame fled north
Echo Knowledge: "Something magical awoke the wolves"

Session:  3 | Adventure: Wild Sheep Chase
Notes: Party laughed constantly; discovered magic instability
Echo Knowledge: "Is wizard's fault? Or something bigger?"

Session:  4-5 | Adventure: Frozen Sick
Notes: [To be filled in after running...]

```

---

**For detailed adventure prep, NPC information, and mystery tracking: See Chapter 5 (Adventures) and Chapter 4 (DM Resources)**

